import type { DaySumModel } from '../models/operation-chart-data.model';
...
export const groupDays = (
  ...
  for (const day of sortedDays) {
    ...
    if (
      ...
      (!isNextDay && currentGroup.length > 0)
    ) {
      // Сохраняем текущую группу
      const groupSum = currentGroup.reduce((sum, d) => sum + daysData[d], 0);
      grouped.push({
        key: currentGroup[0],
