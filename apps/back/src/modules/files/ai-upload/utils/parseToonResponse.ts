...
export const parseToonResponse = (text: string): ExtractedOperation[] => {
    try {
      ...
      const rows = cleanText.trim().split('\n').filter(row => {
        ...
        if (!trimmed || trimmed.startsWith('//') || trimmed.toLowerCase().startsWith('amount|')) return false;
      ...
      return rows.map(row => {
        ...
        let amount = (parts[0] || '').trim().replace(',', '.').replace(/[^0-9.-]/g, '');
        ...
        else if (typeStr === 'E' || typeStr === 'EXPENSE') operationType = 'EXPENSE';
